...
export default function Broot({socket, my_ip}) {
	...
	useEffect(() => {
		...
		let url='http://'+my_ip+':3001/api/chat/channels?token='+(document.cookie.substring(document.cookie.indexOf('=')+1)||"")
	...
	const getAll_other_users = () => {
		...
		let url='http://'+my_ip+':3001/api/chat/users?token='+(document.cookie.substring(document.cookie.indexOf('=')+1)||"")
		axios.get(url)
		.then(res => {
		  console.log(res.data);
		  if (res.data[0])
